...
const deleteMeter = async (req, res) => {
  try {
    ...
    const statusCode = error.message === 'Meter not found' ? 404 : error.message === 'Cannot delete active meter' ? 400 : 500;
    res.status(statusCode).json({
      success: false,
      message: error.message,
    });
...
const createMeterTenant = async (req, res) => {
  try {
    ...
    const statusCode = error.message.includes('already assigned') || error.message.includes('overlap') ? 409 : 
...
const updateMeterTenant = async (req, res) => {
  try {
    ...
    const statusCode =
      ...
      error.message.includes('inactive') ? 400 :
      error.message.includes('overlap') ? 409 : 500;
    res.status(statusCode).json({
      success: false,
      message: error.message,
    });
