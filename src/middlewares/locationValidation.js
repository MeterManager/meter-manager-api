...
const createLocationValidation = [
  ...
  body('address').optional().trim().isLength({ max: 1000 }).withMessage('Address must not exceed 1000 characters'),
...
const getLocationByIdValidation = [param('id').isInt({ min: 1 }).withMessage('Location ID must be a positive integer')];
